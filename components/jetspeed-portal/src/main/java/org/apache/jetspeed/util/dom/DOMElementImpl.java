...
/**
 * Note: Because the DOMElement of dom4j (v1.6.1) is built on DOM Level2 API, it does not have implementations on setTextContent() and getTextContent() of
 * org.w3c.dom.Node interface. However, in Java 1.5 environment, we need to allow the methods invocations. Also, because dom4j DOMElement does not support
 * getOwnerDocument(), we need to provide the method to create nodes with document.
 * 
 * @version $Id$
 */
